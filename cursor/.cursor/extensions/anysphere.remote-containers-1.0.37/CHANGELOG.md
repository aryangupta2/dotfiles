...
# 1.0.37
- Fix remote server installation on WSL - install via FTP

# 1.0.36
- Fix issue with `DISPLAY` during X11 forwarding

# 1.0.35
- Fix an issue where `initializeCommand` could run again when reattaching to an already-running dev container after window reloads or reconnects.

# 1.0.34
- Forward the host `DISPLAY` environment variable into devcontainers on Linux/WSL for X11 GUI applications (defaults to `:0`)

# 1.0.33
- Add support for non-Docker container engines (e.g. Podman)
- Add `${localEnv:VAR}` variable interpolation for devcontainer configurations
- Extend GPGForwarding for containers on remote instances

# 1.0.32
- Refactor GPG agent forwarding on Windows to use the assuan protocol

# 1.0.31
- Add GPG agent forwarding support for commit signing inside dev containers
  - New setting `dev.containers.enableGPGAgentForwarding` (default: true) to enable/disable GPG agent forwarding
  - Automatically exports and imports GPG public keys and ownertrust into containers
  - Forwards the host `gpg-agent` socket into Docker and Kubernetes containers
  - Supports Windows GPG4Win Assuan sockets with proper nonce authentication
